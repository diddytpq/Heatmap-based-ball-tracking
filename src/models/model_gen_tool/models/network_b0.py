...
class EfficientNet(nn.Module):
    def __init__(self, width_coeff, depth_coeff,
        ...
        self.Up_Block_1 = nn.Sequential(
                            nn.Conv2d(432, 260, 1, stride=1, bias=False),
        ...
        self.Up_Block_2 = nn.Sequential(
                            nn.Conv2d(176, 92, 1, stride=1, bias=False),
        ...
        self.Up_Block_3 = nn.Sequential(
                            nn.Conv2d(72, 40, 1, stride=1, bias=False),
        ...
        self.Up_Block_4 = nn.Sequential(
                            nn.Conv2d(48, 24, 1, stride=1, bias=False),
                            ...
                            nn.Conv2d(24, 24, 3, stride=1, padding="same", bias=False),
                            nn.BatchNorm2d(24, momentum=0.99, eps=1e-3),
                            Swish()
                            )

        self.Up_Block_5 = nn.Sequential(
                            nn.Conv2d(27, 8, 1, stride=1, bias=False),
                            nn.BatchNorm2d(8, momentum=0.99, eps=1e-3),
                            Swish(),

                            nn.Conv2d(8, 8, 3, stride=1, padding="same", bias=False),
                            nn.BatchNorm2d(8, momentum=0.99, eps=1e-3),
                            Swish(),
...
if __name__ == "__main__":
    ...
    model = EfficientNet(1, 1).to(device)
    epoch = 150
    time_list =[]
    model.eval()
